...
describe('ProcessManager', () => {
    ...
    it('should limit concurrent executions and queue tasks', async () => {
        ...
        processManager.submitTask({ id: 'task-1', command: 'antigravity', args: [], cwd: '/' });
        processManager.submitTask({ id: 'task-2', command: 'antigravity', args: [], cwd: '/' });

        await new Promise(process.nextTick);

        // Since max concurrency is 1, spawn should only be called once
        expect(mockSpawn).toHaveBeenCalledTimes(1);
        expect(mockSpawn).toHaveBeenCalledWith('antigravity', [], { cwd: '/' });
        ...
        expect(mockSpawn).toHaveBeenLastCalledWith('antigravity', [], { cwd: '/' });
    });
    ...
    it('should kill a running process on stopTask', async () => {
        ...
        processManager.submitTask({ id: 'task-to-kill', command: 'antigravity', args: ['10'], cwd: '/' });
