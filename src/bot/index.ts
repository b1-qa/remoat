...
import { FullQuotaService, formatQuotaTelegram } from '../services/quotaService';
...
export const startBot = async (cliLogLevel?: LogLevel) => {
    ...
    // SECURITY: botToken no longer stored on bridge — used only in Bot() constructor.
    ...
    // /quota command — fetch and display model quota & credits
    bot.command('quota', async (ctx) => {
        try {
            await ctx.reply('⏳ Fetching quota...');
            const _fqs = new FullQuotaService(); const snapshot = await _fqs.fetchFullQuota();
            const text = formatQuotaTelegram(snapshot);
            await ctx.reply(text, { parse_mode: 'Markdown' });
        } catch (err: any) {
            await ctx.reply(`❌ Quota fetch failed: ${err.message}`);
        }
    });

    // /launch command — start/restart/stop Antigravity via PM2
    bot.command('launch', async (ctx) => {
        const arg = ctx.match?.trim();
        try {
            const { exec } = require('child_process');
            const run = (cmd: string): Promise<string> => new Promise((resolve, reject) => {
                exec(cmd, { timeout: 10000 }, (err: any, stdout: string) => err ? reject(err) : resolve(stdout.trim()));
            });

            // Check proxy first
            const proxyUp = await run('ss -tln | grep :1080').catch(() => '');
            if (!proxyUp && arg !== 'stop') {
                await ctx.reply('⚠️ Proxy (port 1080) is DOWN. Connect the SSH tunnel from your Mac first.');
                return;
            }

            if (arg === 'stop') {
                await run('pm2 stop antigravity');
                await ctx.reply('🛑 Antigravity stopped.');
            } else if (arg === 'restart') {
                await run('pm2 restart antigravity');
                await ctx.reply('🔄 Antigravity restarted.');
            } else {
                const status = await run('pm2 pid antigravity').catch(() => '');
                if (status && parseInt(status)) {
                    await run('pm2 restart antigravity');
                    await ctx.reply('🔄 Antigravity restarted.');
                } else {
                    await run('pm2 start antigravity');
                    await ctx.reply('🚀 Antigravity launched.');
                }
            }
        } catch (err: any) {
        ...
        onStart: async (botInfo) => {
            ...
            try {
                await bot.api.setMyCommands([
                    ...
                    { command: 'quota', description: 'Show model quota and credits' },
                    { command: 'launch', description: 'Start/restart/stop Antigravity' },
                ]);
