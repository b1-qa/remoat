...
// SECURITY: Only allow spawning known executables.
// If a new command is needed, add it here explicitly.
...
export class ProcessManager {
    ...
    public async submitTask(options: TaskOptions): Promise<void> {
        // SECURITY: Validate command against allowlist before queuing
        const baseName = path.basename(options.command);
        if (!ALLOWED_COMMANDS.has(baseName)) {
            throw new Error(`ProcessManager: command not in allowlist: ${options.command}`);
        }
